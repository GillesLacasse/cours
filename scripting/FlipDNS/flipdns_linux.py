...
g_ConfigSystemeExploitation = {
    ...
    "LINUX": {
        ...
        "encodage": "utf-8",
        "etats_connexion": ["connected", "connecté"],
        "lire_interface": 'nmcli -t -f DEVICE,STATE device',
        "longueur_fonction_split": 0,
        "lire_dns_interface": 'nmcli dev show {%NOM_INTERFACE%} | grep -E "IP4.DNS|IP6.ADDRESS"',
        "ecrire_dns_interface": 'nmcli con mod "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)" {%AJOUT%}{%VERSION_IP1%}.dns "{%ADRESSE_IP%}" && nmcli con mod "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)" {%VERSION_IP1%}.ignore-auto-dns yes && nmcli con up "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)"',
        "verifier_dns": 'dig A {%DNSTest%} @"{%ServeurDNS%}" -{%STACK_IP%}',
        "activer_dhcp": 'nmcli con mod "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)" {%VERSION_IP1%}.dns "" && nmcli con mod "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)" {%VERSION_IP1%}.ignore-auto-dns no && nmcli con up "$(nmcli -t -f NAME,DEVICE con show | grep ":{%NOM_INTERFACE%}" | cut -d: -f1)"'
   }
}
